...
\begin{itemize}
  \item \textbf{Set up and analyze the inherited WolfBite application.}
  The team cloned, ran, and inspected the existing Flutter/Firebase project so
  later decisions were based on implemented behavior rather than only the
  product description.

  \item \textbf{Map the documented behavior to the implementation.}
  The team finalized 20 code-traceable use cases and connected their primary,
  alternative, and exception paths to the relevant production code.

  \item \textbf{Create and execute automated use-case tests.}
  The team authored 86 tests spanning UC1--UC20. The recorded Project~1a run
  produced 82 passes and four retained failures that became evidence for the
  next version.

  \item \textbf{Document traceability, failures, and coverage.}
  The Project~1a report and supporting records preserve the test results,
  failure classifications, use-case traceability, and the 77.5\%
  executable-line coverage baseline.
\end{itemize}
...
\begin{itemize}
  ...
  choose the reference next step within 60 seconds without facilitator help,
  with success at least 20 percentage points above the baseline. Build a script
  that scores trial logs and runs in CI; check it with known passing and failing
  examples.
...
Build the M0 script and M1's measurement handling and define M4's fixtures and
flow in week 1. In weeks 2--3, implement the bounded basket suggestions while
completing M4 and targeted M3 work. Reserve week 4 for integration and the
evaluation. M4 and M0 are the critical path; if they slip, defer M2 extensions
and nonessential M3 polish. Keep basket suggestions to one-swap previews over
the mock catalog; multi-item optimization and personalized meal planning are
outside this month. Add tests alongside each change, run relevant Project 1a
regressions in CI, and document how another team can run the mock demo and
extend the catalog, explanation rules, basket targets, and checkout-help cases.

\subsection{Future: After the One-Month Project}

\begin{itemize}
  \item \textbf{Permissioned, versioned state APL synchronization.}
  Replace the dated NC APL fixtures used in M4's rejection-recovery scenarios
  with a monitored, permissioned adapter to the live state Authorized Product
  List, recording jurisdiction, data version, refresh timestamp, and known
  conflicts. This directly extends M4's rule-based likely-cause engine by
  giving it real, dated evidence instead of mocked fixtures, but requires
  state data-sharing permission and an operational update owner, which is
  why it stays out of the one-month scope.

  \item \textbf{Partner-backed read-only benefit and rejection-evidence
  pilot.}
  Pilot the M0/M4 checkout-help workflow using authorized participant benefit
  and real transaction-rejection evidence from a willing WIC agency, EBT
  processor, or retailer partner, without allowing WolfBite to approve
  purchases, process payments, or modify benefit balances. This would let the
  team validate whether M4's rule-based causes match real rejection reasons,
  moving the prototype from synthetic scenarios toward evidence-backed
  recovery guidance. It depends on securing a data-sharing agreement and a
  confidentiality/security review with a program partner.

  \item \textbf{Privacy-reviewed on-device receipt recognition and
  reconciliation.}
  Extend M4's item-level recovery flow with on-device receipt scanning,
  reconciling actual purchased items and quantities against the basket and
  benefit records without uploading a full receipt image to a third-party OCR
  service. This responds to the receipt-privacy concern identified in the D2
  stakeholder analysis for outside data-service providers, and requires
  resolving device-support, receipt-format variability, and duplicate/quantity
  matching that are out of scope for the one-month build.

  \item \textbf{Spanish-language and broader accessibility validation.}
  Add professionally reviewed Spanish-language content across the nutrition
  explainers (M1), basket-balance suggestions (M2), redesigned interface (M3),
  and rejection-recovery flow (M4), and validate all four against a wider
  range of disability, literacy, language, device, and network conditions than
  M3's baseline WCAG~2.2 Level AA pass. This extends the D2 stakeholder
  finding that participants with disabilities or limited English proficiency
  could be blocked by camera-only controls or unclear language, but requires
  translation expertise and broader participant recruitment beyond the
  one-month timeframe.

  \item \textbf{Production hardening and operational release.}
  Add the access-control testing, security review, monitoring,
  backup/retention policies, and deployment-specific compliance work needed to
  move WolfBite from a tested student prototype into a safely operable public
  release, including resolving the dependency-licensing and lockfile gaps
  identified in the Project~1a baseline audit. This milestone only becomes
  relevant once M0's evaluation confirms the checkout-help workflow is worth
  deploying beyond the course prototype.
\end{itemize}
...
earlier wording they received. The finalized Gemini and local P10 outputs are
usable but less reproducible: Gemini's saved file omits the pasted prompt text,
and the local transcript contains a prompt placeholder rather than the exact
text.
...
Project 1b identified a medium-confidence opportunity to help WIC participants
when an app marks an item as eligible but checkout rejects it. WICShopper, myWIC
Mosaic, and Bnft cover much of the shopping process, but our review found no
documented item-level explanation and recovery workflow.

Using four LLMs helped us find and compare more competitors, while our own
checks removed claims we could not verify. Prompt~10 did not give us a reason to
change projects, but it warned that our plan may be too large for one month and
that WolfBite cannot access official checkout data. Other milestone and
feasibility prompts showed that the project should still be manageable if we
use mock scenarios, set clear goals, and focus first on M4 and M0. In
Project~2, we will follow this plan, cut lower-priority work if needed, keep our
Project~1a tests working, and evaluate whether the checkout-help feature is
actually useful.
