...
\begin{itemize}
  ...
  Replace static eligibility fixtures with a monitored adapter that records
  jurisdiction, version, refresh time, conflicts, and stale-state information.

  \item \textbf{Partner-backed read-only evidence pilot.}
  Evaluate recovery with authorized participant or transaction evidence without
  ...
  reconciliation.}
  Use local recognition where practical, connect item and quantity evidence to
  basket and benefit information, and retain a manual fallback.

  \item \textbf{Spanish and broader accessibility validation.}
  Add professionally reviewed Spanish content and test disability, literacy,
  language, device, and network conditions.

  \item \textbf{Production hardening and operational release.}
  Add access-control tests, monitoring, backups, retention and deletion
  policies, security review, and deployment-specific compliance work after
  scope and partnerships are stable.
\end{itemize}
